...
## Jira Project used for tracking Red Hat Bugzillas 
REDHAT_PROJECT = "RHBZ"
...
def lookup_proxy(options, bug):
    ...
    payload = {'jql': 'project = ' + REDHAT_PROJECT + ' and summary ~ \'RHBZ#' + str(bug.id) +'\'', 'maxResults' : 5}
...
# set up issue link to RHBZ
def create_remotelink(jira_id, bug):
    link_dict = { "title": "Red Hat Bug #" + str(bug.id), "url": bug.weburl }
    if (options.verbose):
...
def create_proxy_jira_dict(options, bug):
        ...
        if (jiraversion and jiraversion != NO_VERSION): 
            ...
            if accept.capitalize() in "Y":
                print norm + "[WARNING] Version '" + green + jiraversion + norm + "' mapped from '" + \
                    green + bug.target_milestone + norm + "' not found in " + green + REDHAT_PROJECT + \
                    norm + ". Please create it or fix the mapping. " + blue + "Bug: " + str(bug) + norm
                newv = jira.create_version(jiraversion, REDHAT_PROJECT)
                versions = jira.project_versions(REDHAT_PROJECT)
                jiraversion = newv.name
            else:
                print red + "[ERROR] Version '" + green + jiraversion + norm + "' mapped from '" + \
                	green + bug.target_milestone + red + "' not found in " + green + REDHAT_PROJECT + \
    ...
    if (not next((c for c in components if bug.product == c.name), None)):
        ...
        if accept.capitalize() in "Y":
            comp = jira.create_component(bug.product, REDHAT_PROJECT)
            components = jira.project_components(REDHAT_PROJECT)
    ...
    issue_dict = {
        'project' : { 'key': REDHAT_PROJECT },
        'summary' : bug.summary + ' [RHBZ#' + str(bug.id) + "]",
...
bz2jira_resolution = {
           ...
           "NOT_REDHAT" : "Invalid" # don't have an exact mapping so using invalid as "best approximation"
    }
...
# https://bugzilla.redhat.com/buglist.cgi?chfieldfrom=3h&status_whiteboard=RHT&order=changeddate%20DESC%2C
